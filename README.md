...
4. The model replies with either a short answer or a checklist when the job genuinely takes several steps, and names the one control you need to touch.
...
`npm run qa` is the Playwright renderer suite. It checks that the interface
renders, and it is not a test of whether the product does what it claims.
